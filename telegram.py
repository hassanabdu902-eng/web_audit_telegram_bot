...
# chat_id -> ScanJob. Explicit job state prevents stale "already running"
# state after cancellation, timeout, exceptions, or shutdown.
ACTIVE_SCANS = {}
...
class ScanCancelled(Exception):
    """Raised when a scan is explicitly cancelled."""
...
async def status_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"Tracked scans: {len(ACTIVE_SCANS)}"
    )
...
async def run_scan(update: Update, url: str, job: ScanJob):
    if not update.message:
        return
    ...
    try:
        ...
        # A waiting job remains cancellable without occupying the semaphore.
        while True:
            if job.cancel_event.is_set():
                raise ScanCancelled("Scan cancellation requested.")

            try:
                await asyncio.wait_for(SCAN_LIMITER.acquire(), timeout=1.0)
        ...
        async def heartbeat(elapsed):
            ...
            # The scanner returns one complete report rather than per-check
            # events, so this progress value is deliberately approximate.
            estimated = min(
                len(PROGRESS_NAMES) - 1,
                max(
                    1,
                    int(
                        (elapsed / max(1, MAX_SCAN_SECONDS))
                        * len(PROGRESS_NAMES)
                    ),
                ),
            )
        ...
        def heartbeat_from_thread(elapsed):
            if ASYNCIO_LOOP is None or job.cancel_event.is_set():
                return
            try:
        ...
        # Keep the worker future as its own Task. If Telegram cancels this
        # coroutine during shutdown, we signal the process and wait for the
        # worker thread to finish cleanup instead of leaving stale state.
        worker_future = asyncio.create_task(
            asyncio.to_thread(
        ...
        if job.cancel_event.is_set():
            try:
                ...
                pass
            raise ScanCancelled("Scan cancelled before report delivery.")
        ...
        if status_message:
            await safe_edit(
                ...
                "The scanner process was stopped and the chat is ready "
                "for a new scan.",
            )
        ...
        if status_message:
            await safe_edit(
                ...
                "The scan state has been cleared.",
            )
        else:
            try:
                await update.message.reply_text(
        ...
        # Only delete the job that this task created. An old task can never
        # delete a newer scan's state.
        current = ACTIVE_SCANS.get(chat_id)
        if current is job:
            ACTIVE_SCANS.pop(chat_id, None)

        logger.info(
            "Scan state cleared | chat_id=%s | url=%s",
            chat_id,
            url,
        )


async def start_scan(update: Update, url: str):
    """Create exactly one tracked job per chat and schedule it."""
    if not update.message:
        return
    ...
    existing = ACTIVE_SCANS.get(chat_id)
    ...
    if existing is not None:
        await update.message.reply_text(
            ...
            f"Target: {existing.url}\n"
            "Use /cancel if you want to stop it."
        )
        return

    job = ScanJob(chat_id, url)
    ACTIVE_SCANS[chat_id] = job

    task = asyncio.create_task(run_scan(update, url, job))
    job.task = task
    BACKGROUND_TASKS.add(task)
    ...
    def _done(t):
        ...
        try:
            ...
            logger.exception("Could not inspect scan task.")
            return
        if exc:
            logger.error(
...
async def cancel_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    job = ACTIVE_SCANS.get(chat_id)

    if job is None:
        await update.message.reply_text(
            "ℹ️ No active scan is running in this chat."
        )
        return
...
def main():
    ...
    logger.info("Security audit bot v2.3 starting.")
