...
async def run_scan(update: Update, url: str):
    ...
    async with sem:
        ...
        progress_names = [
            "DNS Lookup", "Security Headers", "TLS Analysis",
            "Subdomain Scan", "Admin Panels", "Port Scan",
            "HTTP Methods", "Technology Detection", "Cookie Security",
            "CORS Configuration", "Directory Listing", "Sensitive Files",
            "API Endpoints", "Source Code Analysis", "Login Security",
            "Security.txt", "Backup Files", "Server Disclosure",
            "Sensitive Data", "Subdomain Takeover",
            "HSTS Quality", "CSP Quality", "TLS Protocols",
            "TLS Certificate", "TLS Cipher Review", "DNSSEC",
            "CAA Record", "MX Records", "SPF Quality", "DMARC Quality",
            "Sitemap Exposure", "Redirect Security", "Mixed Content",
            "External Resource Integrity", "Cache-Control Security",
            "Information Disclosure Headers", "API Documentation",
            "WAF Detection", "WHOIS", "IP Geolocation", "Robots.txt",
            "WordPress Users", "XML-RPC", "WordPress Plugins",
            "WordPress Version", "Email Security", "CT Logs", "Wayback",
            "CVE Check", "Phishing Detection", "Trust Analysis", "CRLF",
            "Open Redirect", "Clickjacking", "VCS Exposure",
            "Command Injection", "File Inclusion", "SQL Injection",
            "XSS Test", "Final Analysis",
        ]

        async def heartbeat(elapsed, maximum):
            # scan_url currently returns the complete result only when all
            # checks finish. Therefore the counter below is a time-based
            # activity indicator, not a claim that exactly N checks completed.
            estimated = min(59, max(1, int((elapsed / max(1, maximum)) * 59)))
            current = progress_names[min(estimated, len(progress_names) - 1)]
            completed_names = progress_names[:estimated]
            recent = completed_names[-4:]
            lines = [
                "🔎 Security Audit",
                "",
                "━━━━━━━━━━━━━━━━━━",
                f"Progress: ~{estimated}/60",
                "━━━━━━━━━━━━━━━━━━",
            ]
            for name in recent:
                lines.append(f"✅ {name}")
            lines.append(f"🔄 {current}")
            lines.append(f"⏳ Remaining: ~{60 - estimated} checks")
            lines.append("")
            lines.append(f"Elapsed: {format_elapsed(elapsed)}")
            lines.append("🟢 Scanner is still running")
            await safe_edit(msg, "\n".join(lines))

        # Capture the event loop before entering asyncio.to_thread().
        # The heartbeat callback runs in the worker thread, where
        # asyncio.get_running_loop() is not available.
        event_loop = asyncio.get_running_loop()

        def heartbeat_from_worker(elapsed, maximum):
            # Called from the worker thread while waiting for the child process.
            try:
                future = asyncio.run_coroutine_threadsafe(
                    heartbeat(elapsed, maximum), event_loop
                )
